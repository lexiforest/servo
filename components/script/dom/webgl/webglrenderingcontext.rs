...
impl WebGLRenderingContext {
    ...
    pub(crate) fn new_for_worker(
        ...
        match WebGLRenderingContext::new_inherited_with_channel(
            ...
            Ok(ctx) => Some(reflect_dom_object(Box::new(ctx), global, can_gc)),
            Err(msg) => {
...
impl WebGLRenderingContextMethods<crate::DomTypeHolder> for WebGLRenderingContext {
    ...
    fn GetParameter(&self, cx: SafeJSContext, parameter: u32, mut retval: MutableHandleValue) {
        ...
        match parameter {
            ...
            constants::RENDERER => {
                let value = pref!(bimp_js_webgl_masked_renderer);
                if value.is_empty() {
                    "WebKit WebGL".safe_to_jsval(cx, retval, CanGc::deprecated_note());
                } else {
                    value.safe_to_jsval(cx, retval, CanGc::deprecated_note());
                }
                return;
            },
            constants::VENDOR => {
                let value = pref!(bimp_js_webgl_masked_vendor);
                if value.is_empty() {
                    "WebKit".safe_to_jsval(cx, retval, CanGc::deprecated_note());
                } else {
                    value.safe_to_jsval(cx, retval, CanGc::deprecated_note());
                }
                return;
            ...
            WEBGLDebugRendererInfoConstants::UNMASKED_RENDERER_WEBGL => {
                let value = pref!(bimp_js_webgl_renderer);
                if value.is_empty() {
            ...
            WEBGLDebugRendererInfoConstants::UNMASKED_VENDOR_WEBGL => {
                let value = pref!(bimp_js_webgl_vendor);
                if value.is_empty() {
                    "Google Inc. (Apple)".safe_to_jsval(cx, retval, CanGc::deprecated_note());
                } else {
                    value.safe_to_jsval(cx, retval, CanGc::deprecated_note());
                }
                return;
