    ...
    fn GetParameter(&self, cx: &mut JSContext, parameter: u32, mut retval: MutableHandleValue) {
        ...
        match parameter {
            ...
            constants::RENDERER => {
                ...
                if value.is_empty() {
                    "WebKit WebGL".safe_to_jsval(cx, retval);
                } else {
                    value.safe_to_jsval(cx, retval);
                }
                return;
            },
            constants::VENDOR => {
                ...
                if value.is_empty() {
                    "WebKit".safe_to_jsval(cx, retval);
                } else {
                    value.safe_to_jsval(cx, retval);
                }
                return;
            ...
            WEBGLDebugRendererInfoConstants::UNMASKED_VENDOR_WEBGL => {
                ...
                if value.is_empty() {
                    "Google Inc. (Apple)".safe_to_jsval(cx, retval);
                } else {
                    value.safe_to_jsval(cx, retval);
                }
                return;
